% Code to reproduce normalised visitation rates as a function of free or forced choice conditions (Figure 2A)
...
% save data as csv file to plot simplex in a different script
...
%writetable(T,['csv_output' fs 'proportion_visits.csv']);
...
hold on
agent_means=Fig2A_agent();
agent_means=squeeze(mean(agent_means));
agent_col=[0.4,0.4,0.4];

for add_mean=1:3
    plot([add_mean-0.09 add_mean-0.01],[agent_means(add_mean) agent_means(add_mean)], 'color',agent_col,'LineWidth',5); % 2.5 prev line width
