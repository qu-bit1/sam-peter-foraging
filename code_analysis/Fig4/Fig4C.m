% Code to reproduce arrival reward, as a function of the patch being left, under free or forced choice conditions (Figure 4C)
...
writetable(T,['csv_output' fs 'arrival_reward_patch_being_left.csv']);
...
settings.dolegend=0;
[h,dat_means, dat_SDs, stat_out]=create_lineplot(dat_plt,settings,'Reward on Arrival at Next Patch');

% add lines from simulated means
hold on
agent_means=Fig4C_agent();
...
mvSlw=mean([dat_plt(:,1) dat_plt(:,4)],2);
mvMid=mean([dat_plt(:,2) dat_plt(:,5)],2);
mvFst=mean([dat_plt(:,3) dat_plt(:,6)],2);
[h,pFstMid,CI,statFstMid]=ttest(mvFst,mvMid); pFstMid=pFstMid*3;  % get Bonferroni corrected p (Jafari & Ansari-Pour, 2019).
[h,pFstSlw,CI,statFstSlw]=ttest(mvFst,mvSlw); pFstSlw=pFstSlw*3;
[h,pMidSlw,CI,statMidSlw]=ttest(mvMid,mvSlw); pMidSlw=pMidSlw*3;

% store results
stats.FstMid.p=pFstMid; stats.FstMid.df=statFstMid.df; stats.FstMid.tstat=statFstMid.tstat;
stats.FstSlw.p=pFstSlw; stats.FstMid.df=statFstSlw.df; stats.FstMid.tstat=statFstSlw.tstat;
stats.MidSlw.p=pMidSlw; stats.FstMid.df=statMidSlw.df; stats.FstMid.tstat=statMidSlw.tstat;
...
analysisname='Fig4C_arrivalFromPatch';
