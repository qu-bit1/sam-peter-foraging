% Code to reproduce reward when arriving at each patch as a function of free or forced choice conditions (Figure 4D)
...
% mean difference for each patch
stats.slwCDiff.mean=mean([dat_plt(:,4)-dat_plt(:,1)]); 
stats.midCDiff.mean=mean([dat_plt(:,5)-dat_plt(:,2)]);
stats.fstCDiff.mean=mean([dat_plt(:,6)-dat_plt(:,3)]);
...
% SD difference for each patch
stats.slwCDiff.SD=std([dat_plt(:,4)-dat_plt(:,1)]); 
stats.midCDiff.SD=std([dat_plt(:,5)-dat_plt(:,2)]);
stats.fstCDiff.SD=std([dat_plt(:,6)-dat_plt(:,3)]);
...
analysisname='Fig4D_arrival2Patch';
