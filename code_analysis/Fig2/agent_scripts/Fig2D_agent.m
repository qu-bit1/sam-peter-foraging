% Code to reproduce decisions to select best alternative patch (replenishment rate).
% The function is used to obtain mean scores which can be overlaid on the
% participant data shown in Figure 2D.
...
for isub=1:nsubs
    ...
    for cond=1:2
        ...
        pat_vec=[0,1,2];
        for ipat=1:3
            % get leave decisions from slow patch
...
%writetable(T,'proportion_choices_replenishment_agent.csv');
...
analysisname='proportion_choose_highest_replenishment_agent';
