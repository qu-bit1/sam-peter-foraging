...
function [dat_plt] = Fig5A_benchmark()
...
for isub=1:nsubs
    ...
    for icond=1:2       
        ...
        switch icond
            case 1
                forced_slowPatRew=vertcat(forced_slowPatRew,mean(cond_dat(sl_idx+1,4))); 
                forced_midPatRew=vertcat(forced_midPatRew,mean(cond_dat(mid_idx+1,4)));
                forced_fastPatRew=vertcat(forced_fastPatRew,mean(cond_dat(fst_idx+1,4)));
                
            case 2
                free_slowPatRew=vertcat(free_slowPatRew,mean(cond_dat(sl_idx+1,4))); 
                free_midPatRew=vertcat(free_midPatRew,mean(cond_dat(mid_idx+1,4)));
                free_fastPatRew=vertcat(free_fastPatRew,mean(cond_dat(fst_idx+1,4)));
        end
    end
...
analysisname='Fig5D_arrival2Patch_agent';
