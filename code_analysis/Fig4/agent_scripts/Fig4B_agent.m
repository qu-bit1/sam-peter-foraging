...
function [dat_plt] = Fig5B_benchmark()
...
for icond=1:2       
        ...
        mid_idx=[]; 
        fst_idx=[]; 
        sl_idx=length(find(cond_dat(:,2)==0  & cond_dat(:,1)==0));
        mid_idx=length(find(cond_dat(:,2)==0 & cond_dat(:,1)==1));
        fst_idx=length(find(cond_dat(:,2)==0 & cond_dat(:,1)==2));
        
        sl_act=length(find(cond_dat(:,2)==1 & cond_dat(:,1)==0));
        mid_act=length(find(cond_dat(:,2)==1 & cond_dat(:,1)==1));
        fst_act=length(find(cond_dat(:,2)==1 & cond_dat(:,1)==2));
             
        switch icond
            case 1
                forced_slowPatRew=vertcat(forced_slowPatRew,sl_idx/sl_act); % x exploit actions (sl_idx), spread across y patch visits (slw_act)
                forced_midPatRew=vertcat(forced_midPatRew,mid_idx/mid_act);
                forced_fastPatRew=vertcat(forced_fastPatRew,fst_idx/fst_act);
                
            case 2
                free_slowPatRew=vertcat(free_slowPatRew,sl_idx/sl_act);
                free_midPatRew=vertcat(free_midPatRew,mid_idx/mid_act);
                free_fastPatRew=vertcat(free_fastPatRew,fst_idx/fst_act); 
        end
    end
...
analysisname='Fig6_actsB4Leaving_benchmarks';
