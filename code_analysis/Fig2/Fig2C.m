% Code to reproduce decisions to select best alternative patch (expected reward) as a function of free or forced choice conditions (Figure 2C)
...
for isub=1:nsubs
    ...
    for cond=1:2
                        ...
                        case 0.0500 % patch being left
                            ...
                            switch t(c,2)
                                case 0.1000 % patch arrival
                                    if t(c,3)>t(c,4)
                                        t(c,5)=1; % (switched to highest reward magnitude patch)
                                    end 
                                case 0.1500
                                    if t(c,4)>t(c,3)
                                        t(c,5)=1; % (switched to highest reward magnitude patch)
        ...
        %if the final choice not recorded due to the block ending, remove
        %from dataframe
            t(find(t(:,2)==0),:)=[];
...
writetable(T,['csv_output' fs 'proportion_choices_reward.csv']);
...
% add lines for simulated agent means
hold on
agent_means=Fig2C_agent();
agent_means=squeeze(mean(agent_means));
agent_col=[0.4,0.4,0.4];

for add_mean=1:3
    plot([add_mean-0.09 add_mean-0.01],[agent_means(add_mean) agent_means(add_mean)], 'color',agent_col,'LineWidth',5); % prev size
